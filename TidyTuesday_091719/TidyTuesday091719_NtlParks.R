...
Graph2015<-Summary2015 %>%
  ...
  theme(
    ...
    panel.grid = element_blank(),
    plot.margin = unit(rep(-2,4), "cm"))+
  geom_text(data=label_data, aes(x=state, y=sum, label=state, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=2.5, angle=label_data$angle, inherit.aes = FALSE )
Graph2015
